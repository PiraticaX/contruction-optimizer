...
def solve_quantum(df, max_workers=15):
    ...
    # 🔥 Horizon cap to prevent combinatorial explosion
    horizon = min(40, int(sum(df["duration"])))

    # Penalty weights
    A = 40   # single start constraint
    B = 60   # precedence constraint
    alpha = 1.0
    beta = 0.00001
    ...
    for i, task in enumerate(tasks):
        for t in range(horizon):
            ...
            Q[(var(i, t), var(i, t))] = Q.get(
                (var(i, t), var(i, t)), 0
            ) + duration_term + cost_term
            ...
            for i in preds:
                ...
                for t_i in range(horizon):
                    for t_j in range(horizon):
                        if t_j < t_i + dur_i:
                            Q[(var(i, t_i), var(j, t_j))] = Q.get(
                                (var(i, t_i), var(j, t_j)), 0
                            ) + B
    ...
    response = sampler.sample_qubo(Q, num_reads=150)
    ...
    # Safety fallback (if annealer fails to assign start)
    if not schedule:
        return 999, 9999999, {}
